...
create_datasets <- function(test_writers, d) {
  ...
  row_writer <- as.data.frame(cbind(
    c(1:dim(d_writers2)[1]),
    d_writers2$writer
  )) %>%
...
calc_dist <- function(data, combinations) {
  ...
  for (i in c(1:dim(pairs)[1])) {
    ...
    # Measures ----------------------------------------------------------------
    euch_dist <- dist(rbind(
      samp[pairs[i, 1], as.character(c(1:40))],
      samp[pairs[i, 2], as.character(c(1:40))]
    ))
...
# Set formulae
formula1 <- "factor(y) ~ euch_dist + cos_similarity + spearman"
formula2 <- paste(
  "factor(y) ~",
  paste(
    c(

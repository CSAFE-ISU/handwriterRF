...
test_that("Make densities works with ranger package", {
  ...
  # If running pre-R 4.4.0, stats::density() called by make_densities() does not
  # have the field "old.coords" but the fixture does have this field.
  if (!("old.coords" %in% names(actual$diff_writer))){
    actual$diff_writer$old.coords <- FALSE
  }
  if (!("old.coords" %in% names(actual$same_writer))){

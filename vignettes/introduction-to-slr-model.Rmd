...
We estimated the writer profiles for all handwriting samples using the `get_writer_profiles()` function and the `templateK40` cluster template from `handwriterRF`. Behind the scenes, `get_writer_profiles()` performs the following steps on each handwriting sample:
...
3. Calculate the number of graphs assigned to each cluster with `handwriter::get_cluster_fill_counts()`.
